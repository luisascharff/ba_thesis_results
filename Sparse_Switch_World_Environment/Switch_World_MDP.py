...
class SwitchWorldContinuousSA(gym.Env):
    ...
    def __init__(self, size=1.5, num_switches=2, switch_width=0.3, switch_locations=[np.array([0.3, 0.3]), np.array([1.0, 1.0])], render_mode=False):
        ...
        self._first_switch_toggled = False  # Track if the first switch has been toggled
        ...
        self.action_space = spaces.Box(low=np.array([-1, -1, 0]), high=np.array([1, 1, 1]), dtype=float)  # x, y movement and toggle
    ...
    def reset(self, seed=None, state=None):
        ...
        self._first_switch_toggled = False  
        ...
        if toggle > 0.5: 
            for i, loc in enumerate(self._switch_locations):
                if np.linalg.norm(pos - loc) < self._switch_width and self._can_toggle(i, switches):
                    if i == 0:  
                        if switches[i] == 0 and not self._first_switch_toggled:  
                            reward += 1  
                            self._first_switch_toggled = True  
                        elif switches[i] == 1:  
                            reward -= 1  
